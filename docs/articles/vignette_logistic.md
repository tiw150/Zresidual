...
the `brms` package in R \[@Burkner2017-brms\] to reveal potential model
misspecifications. The examples illustrate the practical use of these
residuals for RPP \[@Feng2020\] diagnostics.
...
$`y_i = 0`$ indicates a failure for the \$i^\th\$ observations.
...
observed value, which can refer to either success or failure. Then, the
Z-residual of the response variable is,
``` math
\begin{equation}
\label{eq:z_residual}
...
\end{equation}
```
where $`\Phi^{-1}(.)`$ is the quantile function of standard normal
distribution.
...
#               data = data_logit, 
#               family = bernoulli())
load("~/Desktop/Zresidual/vignettes/fit_logit.Rdata")
```
...
![](vignette_logistic_files/figure-html/unnamed-chunk-6-1.png)
...
![](vignette_logistic_files/figure-html/unnamed-chunk-7-1.png)
