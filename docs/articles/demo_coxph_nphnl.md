# Z-residuals Diagnostics for Cox Proportional Hazards Models
...
This vignette demonstrates how to use the `Zresidual` package to
reproduce the core workflow of the Cox proportional hazards paper:
...
4.  illustrate the contrast between **functional-form misspecification**
    and **time-varying effects** through simulation.

The practical interpretation is:

- **non-significant Z-SW + significant Z-AOV-LP**: more consistent with
  functional-form misspecification,
- **significant Z-SW + non-significant Z-AOV-LP**: more consistent with
...
We use the Mayo Clinic PBC data from the `survival` package. Following
the paper, transplantation is treated as right censoring for the
...
Log-bilirubin model. Columns (left to right): Q-Q plot, Scatterplot
vs. LP, Boxplot vs. LP.
...
![](demo_coxph_nphnl_files/figure-html/fig-pbc-pvalue-hist-1.png)

Figure 3: Histograms of replicated Z-residual test p-values for the PBC
models. Red lines indicate the conservative upper-bound p-values based
on the median order statistic.
...
In this setting, the data are generated under a proportional-hazards
model with a **log(x2)** effect, but we first fit a misspecified model
using raw `x2`.
...
Figure 4: A comprehensive diagnostic analysis using Z-residuals from
simulated data. The upper row illustrates model diagnostics for a poorly
specified linear-form fit. The lower row shows diagnostics for a
correctly specified nonlinear model incorporating log-transformed
covariates.
...
We perform a simulation loop across 500 datasets to compare the p-values
produced by the Shapiro-Wilk diagnostic test for both the misspecified
linear model and the correctly specified log-nonlinear model. We save
these computation results to avoid repeating them in future renders
unless `force_rerun` is enabled.
...
![](demo_coxph_nphnl_files/figure-html/replication-hists-sw-1.png)

Distributions of Shapiro-Wilk diagnostic test p-values across 500
simulated datasets.

**Discussion:** For the **Correct Model** (log-nonlinear), the
distribution of p-values is relatively uniform, indicating a
well-specified functional form that yields normally distributed
Z-residuals. For the **Wrong Model** (linear), the vast majority of
p-values are clustered near 0, demonstrating that the Shapiro-Wilk test
reliably detects the non-normality caused by the misspecified functional
form.
