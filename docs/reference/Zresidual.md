...
\`Zresidual()\` is an S3 generic for computing Z-residuals for a range
of survival and regression models. It currently supports Cox
...
regression models fitted with \*\*brms\*\* (\`brmsfit\`) for several
common count and hurdle families.

The function inspects the fitted object, assigns an internal model– and
package–specific class (e.g. \`"coxph.survival"\`,
...
- object:
  ...
  \* \`"survreg"\` – Parametric survival regression (from
  \*\*survival\*\*) \* \`"brmsfit"\` – Bayesian regression model (from
  \*\*brms\*\*) with one of the supported families listed under
  \*\*Details\*\*.
...
- type:
  ...
  (mainly used for \*\*brms\*\* and other Bayesian/count-model
...
- method:
  ...
  Common options include \`"iscv"\` (importance-sampling
  cross-validated), \`"loocv"\`, and \`"posterior"\`. The default is
  \`"iscv"\`.
...
Internally, \`Zresidual()\` modifies the class of \`object\` to encode
both the model type and the fitting package, and then uses S3 method
dispatch:

\* For \*\*survival\*\* models - \`coxph\` objects receive the class
\`"coxph.survival"\` and are handled by \`Zresidual.coxph.survival()\`,
which further distinguishes between models with and without frailty
terms. - \`survreg\` objects receive the class \`"survreg.survival"\`
...
such as \`Zresidual.hurdle_poisson.brms()\`,
\`Zresidual.negbinomial.brms()\`, \`Zresidual.poisson.brms()\`, or
...
If \`object\` does not match any supported model class, or if a
\`brmsfit\` object uses an unsupported family, \`Zresidual()\` raises an
error.
