...
Poisson distribution using a brms model with \`family =
hurdle_poisson()\`. Z-residuals can be calculated for zeros, counts, or
the overall hurdle distribution, and can be used for model diagnostics.
...
- type:
  ...
  for the truncated Poisson counts, \`"hurdle"\` for the full
