...
when applied to hurdle negative binomial models fitted with
[`brms::brm()`](https://paulbuerkner.com/brms/reference/brm.html) and
`family = hurdle_negbinomial()`. Objects are dispatched here when the
fitted object is a `"brmsfit"` with
...
than calling `Zresidual.hurdle_negbinomial.brms()` explicitly.
...
- ...:
  ...
  [`Zresidual_hurdle_negbinomial_brms()`](https://tiw150.github.io/Zresidual/reference/Zresidual_hurdle_negbinomial_brms.md).
