...
#'
#' @description
#' `Zresidual.hurdle_negbinomial.brms()` is the S3 method for [Zresidual()]
...
#'
#' @param object A `brmsfit` object with hurdle negative binomial family
...
#'     \item `"count"`  — the truncated negative binomial count part;
#'     \item `"hurdle"` — the full hurdle negative binomial model.
#'   }
...
Zresidual.hurdle_negbinomial.brms <- function(object,nrep = 1,data,
  ...
  out <- Zresidual_hurdle_negbinomial_brms(
    fit    = object,
    type   = type,
    method = method,
    n.rep  = nrep, ...
...
#'
#' @description
#' Computes Z-residuals for fitted Bayesian hurdle negative binomial
#' models using a \pkg{brms} model with `family = hurdle_negbinomial()`.
#' Z-residuals can be calculated for zeros, counts, or the overall hurdle
#' distribution, and can be used for model diagnostics.
#'
#' This is an internal workhorse for [Zresidual.hurdle_negbinomial.brms()]
#' and is not intended to be called directly by end users.
#'
#' @param fit A fitted \pkg{brms} model object for a hurdle negative
#'   binomial outcome.
#' @param type Character string specifying which part of the model to
#'   calculate Z-residuals for:
#'   `"zero"` for the hurdle/zero portion,
#'   `"count"` for the truncated negative binomial counts,
#'   `"hurdle"` for the full hurdle-negative binomial model.
#' @param method Character string specifying the residual calculation method:
...
#'         such as \code{\link{log_pred_dist_HNB}} or
#'         \code{\link{log_pred_dist_TNB}}.
...
#'
#' @seealso
#' \code{\link{log_pred_dist_HNB}}, \code{\link{log_pred_dist_TNB}},
