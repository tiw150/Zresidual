...
#' size <- 1.5
#' pi <- 0.3
#' phurdlenb(y, mu, size, pi)
#'
#' # Upper tail probabilities on log scale
#' phurdlenb(y, mu, size, pi, lower.tail = FALSE, log.p = TRUE)
#'
#' @seealso
#' \code{\link[stats]{pnbinom}}, \code{\link[stats]{dnbinom}},
#' \code{\link{dtruncnb}} for the zero-truncated negative binomial PMF.
#' @export
phurdlenb <- function(y, mu, size, pi, lower.tail = FALSE, log.p = FALSE)
{
