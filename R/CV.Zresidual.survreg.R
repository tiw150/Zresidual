cv.zresidual.survreg<- function( fit, data, nfolds=NULL,foldlist=NULL)
{
  # Required packages:
  ...
  for(fid in 1:length (foldlist))
  {
    ...
    fit_traindata <- tryCatch(
      survreg(fit$terms,data=data.train,dist=fit$dist),
      error = function(e) NA,
    ...
    if(any(!is.na(fit_traindata))){
      res_fold[[fid]]<-Zresidual.survreg(survreg_fit=fit_traindata,newdata = data.test)
    }
  }
