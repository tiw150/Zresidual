...
Let $y_i \in \{0, 1\}$, where $y_i = 1$ indicates a success, and $y_i = 0$ indicates a failure for the $i^\th$ observations.

Using Bayesian estimation (e.g., via the `brms` package), we draw $T$ samples from the posterior distribution. Let  $\theta^{(t)}$ denote the $t^\th$ posterior draw.
...
```{r echo=FALSE}
# save(fit_logit, file="./vignettes/vignette_logistic_supportive/fit_logit.Rdata")
# load("./vignette_logistic_supportive/fit_logit.Rdata")
```
...
zres_logit_post <- Zresidual.bernoulli(fit_logit, method = "post")
```
...
plot.zresid(zres_logit_iscv, X="x1", outlier.return = TRUE)
boxplot.zresid(zres_logit_iscv, X="x1")
```

```{r}
par(mfrow = c(1,3))
qqnorm.zresid(zres_logit_post)
plot.zresid(zres_logit_post, X="x1", outlier.return = TRUE)
boxplot.zresid(zres_logit_post, X="x1")
```
...
```{r}
aov.test.zresid(zres_logit_iscv, X="fitted.value")
aov.test.zresid(zres_logit_post, X="fitted.value")
```

**Bartlett Test (`r BL`)**
```{r}
bartlett.test.zresid(zres_logit_iscv, X="fitted.value")
bartlett.test.zresid(zres_logit_post, X="fitted.value")
```
