...
\left\{
\begin{array}{rl}
S_{ij}(y_{ij}), & \text{if } y_{ij} \text{ is uncensored, i.e., } \delta_{ij}=1, \\
U_{ij}\,S_{ij}(y_{ij}), & \text{if } y_{ij} \text{ is censored, i.e., } \delta_{ij}=0, 
\end{array}
\right.
$$ {#eq-rsp}
...
#| echo: false
#| fig-align: center
#| fig-cap: "Animated QQ plots of Z-residuals."

gif_name <- "qqplot_anim.gif"
gif_storage_path <- file.path(extdata_path, gif_name)

# 1. Generate only if we are the developer and the file doesn't exist yet
if (is_dev && !file.exists(gif_storage_path)) {
  message("Generating animation GIF...")
  gifski::save_gif(
    expr = {
      for (i in 1:10) {
    ...
    gif_file = gif_storage_path,
    width = 800, height = 400, res = 72, delay = 0.8, loop = TRUE
  )
}

# 2. Retrieve local path (Copies from extdata to current folder if needed)
local_gif <- get_local_asset(gif_name, extdata_path)

# 3. Render the local copy
if (!is.null(local_gif) && file.exists(local_gif)) {
  knitr::include_graphics(local_gif)
} else {
  # Static fallback
  par(mfrow = c(1, 2), mar = c(4, 4, 2, 2))
  qqnorm.zresid(Zresidual::Zresid.LeukSurv.wbc, irep = 1)
  qqnorm.zresid(Zresidual::Zresid.LeukSurv.logwbc, irep = 1)
}
```
