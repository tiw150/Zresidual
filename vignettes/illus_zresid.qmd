...
for (scen in scenarios) {
  ...
  anim_full_path <- file.path(extdata_path, anim_filename) # Ensure extdata_path is defined in your setup
  
  # Only run if needed (logic from previous conversations)
  # Assuming is_source/rerun checks are in your environment, otherwise remove 'if'
  if (TRUE) { 
    ...
    av::av_capture_graphics({
      ...
      for (i in 1:n_sim) {
        ...
        if (i == n_sim) {
          # 1. Save Static Frame for PDF (Full version)
          static_name <- paste0("z_residual_static-", scen$suffix, ".png")
          ggsave(file.path(extdata_path, static_name), plot = p_full, 
