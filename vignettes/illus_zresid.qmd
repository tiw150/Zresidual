...
if (is_source && (rerun || !file.exists(anim_full_path))) {
  ...
  # Height set to 900 to accommodate the taller Top Plot
  save_gif({
    ...
    # Postulated Density (Dashed) - The Expectation
    df_post <- data.frame(
      x = x_nb, 
      prob = dnbinom(x_nb, mu = mu_postulated, size = size_val),
      type = "Postulated"
    )
    
    # True Density (Solid) - The Reality
    df_true <- data.frame(
      x = x_nb, 
      prob = dnbinom(x_nb, mu = mu_true, size = size_val),
      type = "True"
    )
    ...
    for (i in 1:n_sim) {
      ...
      p_nb <- ggplot() +
        ...
        # --- FIX: Use coord_cartesian(clip = "off") ---
        # We remove 'limits' from scale_* and rely on coord_cartesian for framing.
