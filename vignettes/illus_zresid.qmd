...
col_curr <- "#d7191c"   
col_past <- "#fdae61"   
# New color for past RSP points (Lighter than Z_i past color)
col_rsp_past <- "#80cdc1" # Light teal/blue to distinguish from orange Z points
    ...
    av::av_capture_graphics({
      ...
      for (i in 1:n_sim) {
        ...
        p_meter <- ggplot() +
          ...
          # --- NEW: RSP Points (History) ---
          # Plot past points slightly below the bar
          ...
          # Moved Label Lower to make room
          ...
          scale_y_continuous(limits = c(-0.6, 2.5), expand = c(0,0)) + # Adjusted y-limit
        ...
        p_nb <- ggplot() +
          ...
          # --- MODIFIED: Heavier dashed lines (size=1.2) ---
          ...
          scale_fill_manual(name = "Area", values = c("S" = col_S, "U" = col_U), guide="none") +
        ...
        print(plot_grid(p_meter, plots_aligned[[1]], plots_aligned[[2]], ncol = 1, rel_heights = c(0.6, 1.2, 1))) # Adjusted heights
      }
...
::: {#fig-aniz-true}
{{< video z_residual_anim-correct.mp4 >}}
...
::: {#fig-aniz-wrong}
{{< video z_residual_anim-wrong.mp4 >}}
