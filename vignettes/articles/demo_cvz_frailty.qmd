...
subtitle: "Examples of Cross-validatory Z-Residual"
format:
  html:
    code-fold: true
vignette: >
  %\VignetteIndexEntry{Cross-validatory Z-Residual for Diagnosing Shared Frailty Models}
...
resources_path <- "resources/cvz_coxph_frailty"
...
```{r fig-kindy-combined-animation}
#| label: fig-kidney-combined-animation
#| fig-cap: "Scatterplots and QQ plots of ordinary and leave-one-out cross-validated Z-residuals for the shared frailty Cox model."
#| code-summary: "Animated ggplot diagnostic panels (folded)"
#| message: false
#| code-fold: true
...
```{r echo=FALSE}
#| label: shapiro-pvalue-summary

med_pval_sw_nocv <- upper_bound_pvalue(sw.kidney)

med_pval_sw_loocv <- upper_bound_pvalue(sw.kidney.cv)

pvalue_summary <- data.frame(
  method = c("Ordinary Z-residual", "LOOCV Z-residual"),
  p_value_upper_bound = c(med_pval_sw_nocv, med_pval_sw_loocv)
)

knitr::kable(
  pvalue_summary,
  digits = 4,
  caption = "Upper-bound Shapiro-Wilk p-values for ordinary and LOOCV Z-residuals."
)
```

```{r}
#| label: shapiro-pvalue-histograms
#| fig-width: 10
#| fig-height: 4.8
#| fig-align: center
#| fig-cap: "Sampling distributions of Shapiro-Wilk p-values across randomized Z-residual replicates."
