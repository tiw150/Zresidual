...
format:
  html:
    ...
    code-fold: true
vignette: >
  %\VignetteIndexEntry{An Animation for Understanding Z-residuals}
...
#| echo: false
#| eval: true
# Toggle this to TRUE when you want to re-generate the heavy video files
rerun <- FALSE
```
```{r setup-paths}
#| include: false
# Define the local resources path inside the vignettes directory
resources_path <- "resources/illusZ"
dir.create(resources_path, recursive = TRUE)

# Helper function to dynamically embed the video
embed_video <- function(suffix) {
  vid_path <- file.path(resources_path, paste0("z_residual_anim-", suffix, ".mp4"))
  
  # The output must have NO indentation before the ```{=html} tags
  cat(
    "\n```{=html}\n",
    sprintf('<div style="display: flex; justify-content: center; margin-bottom: 10px;">
...
pkgs <- c("dplyr", "patchwork", "cowplot", "grid", "ggplot2", "av")

# Install missing packages using renv (so they stay in your project library)
for (pkg in pkgs) {
...
# Only run the heavy computation if the rerun flag is TRUE
if (rerun) {
  ...
  for (scen in scenarios) {
    ...
    anim_full_path <- file.path(resources_path, anim_filename)
    ...
    av::av_capture_graphics({
      ...
      for (i in 1:n_sim) {
        ...
        p_full <- plot_grid(p_meter, plots_aligned[[1]], plots_aligned[[2]], ncol = 1, rel_heights = c(0.6, 1.2, 1))
        
        print(p_full)
        
        # --- E. Special Save at Final Frame ---
        if (i == n_sim) {
          # Save Static Frame for PDF (Full version)
          static_name <- paste0("z_residual_static-", scen$suffix, ".png")
          ggsave(file.path(resources_path, static_name), plot = p_full, 
                 width = 8, height = 8, dpi = 300, bg="white")
          message("Saved static frame: ", static_name)
        }
      }
