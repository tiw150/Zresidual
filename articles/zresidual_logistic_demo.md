...
Let $y_{i} \in \{ 0,1\}$, where $y_{i} = 1$ indicates a success, and
$y_{i} = 0$ indicates a failure for the $i^{t}h$ observations.
...
For a given observation $y_{i}^{\text{obs}}$, the posterior predictive
PMF and survival functions are defined below.

$$p_{i}^{\text{post},\text{logit}}\left( y_{i}^{\text{obs}} \right) = \frac{1}{T}\sum\limits_{t = 1}^{T}\begin{cases}
{\pi_{i}^{o}}^{(t)} & {{\text{if}\mspace{6mu}}y_{i}^{\text{obs}} = 1,} \\
{1 - {\pi_{i}^{o}}^{(t)}} & {{\text{if}\mspace{6mu}}y_{i}^{\text{obs}} = 0,} \\
0 & \text{otherwise.}
\end{cases}$$

$$S_{i}^{\text{post},\text{logit}}\left( y_{i}^{\text{obs}} \right) = \frac{1}{T}\sum\limits_{t = 1}^{T}\begin{cases}
1 & {{\text{if}\mspace{6mu}}y_{i}^{\text{obs}} < 0,} \\
{1 - {\pi_{i}^{o}}^{(t)}} & {{\text{if}\mspace{6mu}}0 \leq y_{i}^{\text{obs}} < 1,} \\
{0,} & {{\text{if}\mspace{6mu}}y_{i}^{\text{obs}} \geq 1.}
\end{cases}$$
...
the observed value, which can refer to either success or failure. Then,
the Z-residual of the response variable is,
