...
$n_{i}$ individuals, indexed as $i$ = 1, 2, , $g$ in the case of
clustered failure survival data. Let $y_{ij}$ be a possibly
...
normalized randomized survival probabilities (RSPs) for $y_{ij}$ in the
shared frailty model is defined as:
$$S_{ij}^{R}\left( y_{ij},\delta_{ij},U_{ij} \right) = \left\{ \begin{aligned}
{S_{ij}\left( y_{ij} \right),} & {{\text{if}\mspace{6mu}}y_{ij}{\mspace{6mu}\text{is uncensored, i.e.,}\mspace{6mu}}{\delta_{ij} = 1}\text{,}} \\
{U_{ij}\, S_{ij}\left( y_{ij} \right),} & {{\text{if}\mspace{6mu}}y_{ij}{\mspace{6mu}\text{is censored, i.e.,}\mspace{6mu}}{\delta_{ij} = 0}\text{,}}
\end{aligned} \right.$$ where $U_{ij}$ is a uniform random number on
$(0,1)$, and $S_{ij}(\cdot)$ is the postulated survival function for
$t_{ij}$ given $x_{ij}$.
$S_{ij}^{R}\left( y_{ij},\delta_{ij},U_{ij} \right)$ is a random number
between $0$ and $S_{ij}\left( y_{ij} \right)$ when $y_{ij}$ is censored.
It is proved that the RSPs are uniformly distributed on $(0,1)$ given
$x_{i}$ under the true model. Therefore, the RSPs can be transformed
into residuals with any desired distribution. We prefer to transform
them with the normal quantile:
$$r_{ij}^{Z}\left( y_{ij},\delta_{ij},U_{ij} \right) = -\Phi^{-1}\left( S_{ij}^{R}\left( y_{ij},\delta_{ij},U_{ij} \right) \right),$$
...
$t_{ij}^{test}$, is excluded from the dataset with $n$ observations. The
...
$\widehat{\beta\prime}$, and frailty effects, $\widehat{z_{i}}$. The
Breslow estimator helps estimate the cumulative baseline hazard
($\widehat{H_{0}}$). The survival function
$\widehat{S}{ij}\left( y{ij} \right)$ for the test observation
$y_{ij}^{test}$ is computed using:
$${\widehat{S}}_{ij}\left( y_{ij}^{test} \right) = \exp\{-\widehat{z_{i}}\exp\left( \widehat{\beta\prime}x_{ij} \right){\widehat{H}}_{0}\left( y_{ij}^{test} \right)\}.$$
Subsequently, the RSP for the observed $t_{ij}$ is defined as:
$${\widehat{S}}_{ij}^{R}\left( t_{ij}^{test},d_{ij},U_{ij} \right) = \left\{ \begin{aligned}
{{\widehat{S}}_{ij}\left( t_{ij}^{test} \right),} & {{\text{if}\mspace{6mu}}t_{ij}^{test}{\mspace{6mu}\text{is uncensored, i.e.,}\mspace{6mu}}{d_{ij} = 1}\text{,}} \\
{U_{ij}\,{\widehat{S}}_{ij}\left( t_{ij}^{test} \right),} & {{\text{if}\mspace{6mu}}t_{ij}^{test}{\mspace{6mu}\text{is censored, i.e.,}\mspace{6mu}}{d_{ij} = 0}\text{.}}
\end{aligned} \right.$$ Resulting in the Z-residual for $t_{ij}^{test}$:
$${\widehat{z}}_{ij}\left( t_{ij}^{test},d_{ij},U_{ij} \right) = -\Phi^{-1}\left( {\widehat{S}}_{ij}^{R}\left( t_{ij}^{test},d_{ij},U_{ij} \right) \right).$$
By repeating these steps for each observation ($n$ times), a LOOCV
predictive Z-residual is calculated for each observation.
...
of -3 to 3, displaying alignment with the $45^{\circ}$ straight line in
the QQ plot. The QQ plot of the No-CV Z-residuals indicates a SW p-value
of around 0.70, signifying a well-fitted model to the dataset. Thus, the
...
$45^{\circ}$ straight line, attributed to the considerable Z-residuals
of the two identified outliers. The SW p-value of LOOCV Z-residuals is
