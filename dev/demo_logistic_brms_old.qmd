---
title: "Demo of Z-residual Diagnosis for Bayesian Logistic Regression"
vignette: >
...
$$
\text{rpp}_i(y_i^\text{obs} | \theta^{(t)}) = S_i(y_i^\text{obs} | \theta^{(t)}) + U_i \times p_i(y_i^\text{obs} | \theta^{(t)})
$$ {#eq-post_rpp}
...
zres_logit_iscv <- Zresidual(fit_logit, data=data_logit,
                             config=list(method="iscv"), randomized=TRUE, nrep=10)

zres_logit_post <- Zresidual(fit_logit, data=data_logit,
                             config=list(method="post"), randomized=TRUE, nrep=10)

zcov_logit <- Zcov(fit_logit, data=data_logit, config=list(type="zero"))
```
...
gif_iscv_name <- "iscv_qqplot_anim.gif"
gif_iscv_path <- file.path(extdata_path, gif_iscv_name)


if (is_dev && (force_rerun || !file.exists(gif_iscv_path))) {
  gifski::save_gif(
    expr = {
      for (i in 1:10) {
        par(mfrow = c(1, 3))
        
        qqnorm.zresid(zres_logit_iscv,info = zcov_logit,irep=i)
        plot.zresid(zres_logit_iscv,info = zcov_logit, x_axis_var="x2",
                    outlier.return = TRUE,categor=TRUE,irep=i)
        boxplot(zres_logit_iscv,info = zcov_logit, x_axis_var = "lp", irep = i)
        
      }
    },
    gif_file = gif_iscv_path,width = 900, height = 320, res = 96, delay = 0.8
  )
}

local_iscv <- get_local_asset(gif_iscv_name, extdata_path, local_assets_dir)
if (!is.null(local_iscv)) knitr::include_graphics(local_iscv)

```

```{r}
#| label: fig-plot-animation_post
#| fig-cap: "Z-residual Q–Q plots for overall GOF (animation over 10 randomization replicates). Each frame corresponds to one regenerated Z-residual set; the panel title reports the replicate-specific Z-SW p-value."
#| fig-align: center
#| code-summary: "GIF Generation Code (Folded)"
#| code-fold: true
#| fig-alt: "Animated plots of Z-residuals comparing models."

gif_post_name <- "post_qqplot_anim.gif"
gif_post_path <- file.path(extdata_path, gif_post_name)


if (is_dev && (force_rerun || !file.exists(gif_post_path))) {
  gifski::save_gif(
    expr = {
      for (i in 1:10) {
        par(mfrow = c(1, 3))
        
        qqnorm.zresid(zres_logit_post,info = zcov_logit,irep=i)
        plot.zresid(zres_logit_post,info = zcov_logit, x_axis_var="x2",
                    outlier.return = TRUE,categor=TRUE,irep=i)
        boxplot(zres_logit_post,info = zcov_logit, x_axis_var = "lp", irep = i)
        
      }
    },
    gif_file = gif_post_path,width = 900, height = 320, res = 96, delay = 0.8
  )
}

local_post <- get_local_asset(gif_post_name, extdata_path, local_assets_dir)
if (!is.null(local_post)) knitr::include_graphics(local_post)
```
...
res_aov_iscv <- as.numeric(aov.test.zresid(zres_logit_iscv, X="lp",zcov = zcov_logit))
res_aov_post <- as.numeric(aov.test.zresid(zres_logit_post, X="lp",zcov = zcov_logit))

res_bl_iscv <- as.numeric(bartlett.test.zresid(zres_logit_iscv, X="lp",zcov = zcov_logit))
res_bl_post <- as.numeric(bartlett.test.zresid(zres_logit_post, X="lp",zcov = zcov_logit))

gof_wide <- data.frame(
  Residual = paste0("Z-residual ", 1:10),
